...
{  // Grid G is generiekc class 
    public class Grid_Utils<TGridObject>
    {
        ...
        public Grid_Utils(int width, int height, int cellSize, Vector3 orgine, Func<Grid_Utils<TGridObject>, int, int, TGridObject> creatGridTobject)
        {
            ...
            for (int x = 0; x < gridArray.GetLength(0); x++)
            {
                for (int y = 0; y < gridArray.GetLength(1); y++)
                {
                    ...
                    string text = "";
                    int fontsize = 5;
                    _Utils.createTextInWorld((name), parent, Localposition, text, fontsize, Color.red, TextAnchor.MiddleCenter);
                    //drew lines
                    if (DeBugView)
                    {
                        string Text = gridArray[x, y].ToString();
                        DebugTexArray[x, y] = _Utils.createTextInWorld((name), parent, Localposition, Text, fontsize, Color.white, TextAnchor.MiddleCenter);
        ...
        private void CreatNewTGridObjectForArray(Func<Grid_Utils<TGridObject>, int, int, TGridObject> creatGridTobject)
        {
            for (int x = 0; x < gridArray.GetLength(0); x++)
            {
                for (int y = 0; y < gridArray.GetLength(1); y++)
        ...
        public Grid_Utils(int cellsize, string tag)
        {
            this.cellSize = cellsize;
        ...
        #endregion
        #region Get
        public Vector3 GetWorldPosition(int x, int y) // local postion to world positon 
        {
            return new Vector3(x, y) * cellSize + orgine;
        }
        public void GetXY(Vector3 worldpositon, out int x, out int y) // world postion to local pisiton
        {
